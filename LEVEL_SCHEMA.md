...
### 선택 확장: 능력 유지와 공중 통과 판정

아래 필드는 version 1·2 모두 선택적으로 사용한다. 생략하면 기존 동작을 유지한다.
...
`activationTop: 0`이면 높은 경로도 판정한다. 변신 아이템은 아이템 원화만 표시하며 판정 범위의 설명 문구나 경계선을 그리지 않는다. 체크포인트의 통과 판정에는 빛기둥을 표시하고 활성화 후 숨긴다. 체크포인트의 `x,y`에는 몸 전체가 들어갈 안전한 지상 공간을 확보해야 하며, 이미 저장된 지점보다 진행 방향상 뒤에 있는 체크포인트는 저장 위치를 되돌리지 않는다.

별은 데이터의 `x,y`가 중심이고, 다른 아이템은 `y - 44`가 표시 중심이다. 통과 판정이 없는 아이템은 중심 기준 48×48px, 체크포인트는 `(x, y - 48)` 중심 기준 64×112px 판정이다. `src/data/levelInteractions.js`를 런타임과 배치 검증이 함께 사용한다.
