...
통과 판정이 있는 오브젝트에는 빛기둥을 표시하고 획득·활성화 후 숨긴다. `activationTop: 0`이면 높은 경로도 판정하며, 체크포인트의 `x,y`에는 몸 전체가 들어갈 안전한 지상 공간을 확보해야 한다. 이미 저장된 지점보다 진행 방향상 뒤에 있는 체크포인트는 저장 위치를 되돌리지 않는다.
