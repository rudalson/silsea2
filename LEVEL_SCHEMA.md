...
`activationTop: 0`이면 높은 경로도 판정한다. 변신 아이템과 체크포인트 모두 기존 원화만 표시하며, 넓어진 판정 범위의 설명 문구·빛기둥·경계선을 그리지 않는다. 체크포인트의 `x,y`에는 몸 전체가 들어갈 안전한 지상 공간을 확보해야 하며, 이미 저장된 지점보다 진행 방향상 뒤에 있는 체크포인트는 저장 위치를 되돌리지 않는다.
