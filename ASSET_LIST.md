...
> 뿔·날개·알리콘 아이템과 실제·공중·장난 게이트의 기존 원화 재사용·통합 검수 G6 승인 · 2026-09-09
...
| `decor_flower` | S | S6 완료: `level-01` 유니콘 정원 x 3200, 128×128 투명 PNG·표시 88×88 |
| `decor_grass` | S | S6 완료: `level-01` 튜토리얼 x 640, 192×128 투명 PNG·표시 144×72 |
| `decor_rock` | S | S6 완료: `level-03` 안개 초입 x 576, 160×128 투명 PNG·표시 112×72 |
| `decor_sign` | S | S6 완료: `level-04` 역방향 시작 x 9920, 160×192 투명 PNG·표시 112×144·좌우 반전 |
...
게이트 등장 섬광·팡 원·별 글리터·빛기둥과 장난 게이트의 연기·별은 코드 도형 및 풀링 파티클로 구현한다. 공중 게이트 안내점은 기존 `item_star` 원화를 사용하므로 별도 이미지 키를 추가하지 않는다. 화면 효과 약하게와 이미지 fallback도 같은 진행 판정을 공유한다.
...
| `silsea_wing_guard` | S | 4 | 제작·등록·최종 승인 완료 |
| `potato89_wing_guard` | S | 4 | 제작·등록·최종 승인 완료 |
...
| 키 | 규격 | 용도 | 상태 |
|---|---:|---|---|
| `ui_result_sticker_clear` | 128×128 | 항상 표시되는 무지개·구름 클리어 기호 | 제작·등록·최종 검수 대기 |
| `ui_result_sticker_collect` | 128×128 | `collect_stars`의 노란 5각 별 | 제작·등록·최종 검수 대기 |
| `ui_result_sticker_secret` | 128×128 | `find_secrets`의 파란 열쇠구멍 | 제작·등록·최종 검수 대기 |
| `ui_result_sticker_speed` | 128×128 | `clear_time`의 날개 시계 | 제작·등록·최종 검수 대기 |
| `ui_result_sticker_perfect` | 128×128 | `no_damage`의 하트 방패 | 제작·등록·최종 검수 대기 |

모두 투명 캔버스와 승인 팔레트를 사용한다. 정상 에셋을 읽지 못하거나 `fallback=1`이면 같은 슬롯의 코드 도형으로 대체한다.

### C4 실비아

| 키 | 규격 | 구성 | 상태 |
|---|---:|---|---|
| `sylvia_*` | 128×128/프레임 | 기본형 13개 시트·62프레임 | 제작·manifest 등록·최종 승인 완료 |
| `sylvia_unicorn_*` | 128×128/프레임 | 유니콘 파생 9개 시트·38프레임 | 제작·manifest 등록·최종 승인 완료 |

기본형은 무뿔·무날개이며 변신 시퀀스와 비행 시퀀스에서만 해당 부속물이 나타난다. 전체 22개 시트·100프레임의 접촉 시트는 `references/could4-sylvia-animation-overview.png`에서 확인한다.

### P6 Should와 Could

| 키 | 우선 | 구성 | 상태 |
|---|---|---|---|
| `potato_archer` | S | idle 2, aim 3, shoot 3, defeated 4 | 제작·등록·최종 승인 완료 |
| `projectile_arrow` | S | 화살 1종, 런타임 회전 | 제작·등록·최종 승인 완료 |
| `laser_emitter` | S | 레이저 발사기 1종 | 제작·등록·최종 승인 완료 |
| `laser_switch_on` | S | 켜진 스위치 | 제작·등록·최종 승인 완료 |
| `laser_switch_off` | S | 꺼진 스위치 | 제작·등록·최종 승인 완료 |
| `fx_laser_beam` | S | 반복 빔 | 제작·등록·최종 승인 완료 |
| `fx_laser_warning` | S | 발사 예고 | 제작·등록·최종 승인 완료 |
...
| 키 | 우선 | 구성 | 상태 |
|---|---|---|---|
| `hula_king` | M | idle 4, spin 8, warning 4, throw 6, vulnerable 4, hurt 3, defeated 8 | 제작·등록·최종 승인 완료 |
| `fx_hula_spin` | M | 세 겹 회전 방어 효과 256×112 | 제작·등록·최종 승인 완료 |
| `projectile_hula_hoop_low` | M | 낮은 가로 링 112×64 | 제작·등록·최종 승인 완료 |
| `projectile_hula_hoop_jump` | M | 점프 회피 세로 링 80×112 | 제작·등록·최종 승인 완료 |

### P11 투명대왕

| 키 | 우선 | 구성 | 상태 |
|---|---|---|---|
| `invisible_king` | M | idle 4, reveal 6, hide 6, attack 6, hurt 3, defeated 8 | 제작·등록·최종 승인 완료 |
| `fx_invisible_reveal` | M | 공개 빛기둥 192×256, 6프레임 | 제작·등록·최종 승인 완료 |
| `fx_invisible_afterimage` | M | 위치 기억 구형 잔상 192×192, 4프레임 | 제작·등록·최종 승인 완료 |
| `fx_invisible_miss` | M | 원·십자 실패 공격 256×192, 6프레임 | 제작·등록·최종 승인 완료 |
| `fx_invisible_crown_impact` | M | 피격·격파 왕관 충격 160×112 | 제작·등록·최종 승인 완료 |

### P12 물대왕

| 키 | 우선 | 구성 | 상태 |
|---|---|---|---|
| `water_king` | M | idle 4, submerge 6, emerge 6, attack 6, dizzy 4, hurt 3, defeated 8 | 제작·등록·최종 승인 완료 |
| `fx_water_king_ripple` | M | 웅덩이 예고 파문·기포 256×128, 6프레임 | 제작·등록·최종 승인 완료 |
| `fx_water_king_projectile` | M | 구형 물 투사체 64×64, 4프레임 | 제작·등록·최종 승인 완료 |
