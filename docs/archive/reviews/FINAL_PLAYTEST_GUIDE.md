...
> 상태: G7 기준 실행 절차 동기화 완료 · 2026-09-03 사용자 미실시 예외 승인 · 실제 세션은 여전히 미실시
> 범위: Phase 1·2·4와 G7에 남은 어린이 3명 검증을 현재 여섯 레벨·다섯 보스·지상/공중/장난 게이트 기준으로 한 번에 수행

## 1. 기존 기록과 구분

`test_json/`의 `child-01` 파일은 2026-08-31 P10~P13 보스와 G1~G6 게이트·아이템 통합 전에 수집되어 현재 보스전의 phase·실패 밟기·HP 손실, 장난 게이트 반응과 공중 게이트 정체를 포함하지 않는다. 역사 자료로 보존하되 이번 최종 게이트의 완료 세션으로 세지 않는다. 새 기록은 `final-child-01`~`final-child-03` ID를 사용한다.
...
|---|---|---|---|
| `final-child-01` | 실세아 | 일반·키보드·기본 효과/소리 | 01 → 02 → 03 → 04 → 05 → 06 |
| `final-child-02` | 감자89 | 일반·게임패드·흔들림 Off | 03 → 04 → 05 → 06 → 01 → 02 |
| `final-child-03` | 실세아 | 쉬움·키보드·효과 약하게·음소거 | 05 → 06 → 01 → 02 → 03 → 04 |
...
| `level-06` 무지개 릴레이·공중 게이트 | `?playtest=1&tester=final-child-01&character=silsea&level=level-06` | `?playtest=1&tester=final-child-02&character=potato89&level=level-06&shake=0` | `?playtest=1&tester=final-child-03&character=silsea&level=level-06&easy=1&effects=reduced&mute=1` |
...
7. `level-02`에서는 장난 게이트를 실제 출구로 오해했는지와 사라진 뒤 진행 방향을 찾았는지 적는다.
8. `level-06`에서는 공중 게이트 발견부터 진입까지 20초 이상 멈췄는지, 직접 점프와 보조 발판 중 어떤 경로를 사용했는지 적는다.

관찰 메모는 [FINAL_PLAYTEST_OBSERVATION_SHEET.md](./FINAL_PLAYTEST_OBSERVATION_SHEET.md)에 기록한다.

## 4. 저장과 분석

각 완주 뒤 클리어 화면에서 `E` 또는 결과 저장 버튼을 눌러 JSON을 받는다. 18회가 한 브라우저의 최근 24개 보존 범위 안에 들어가므로 중간에 저장소를 지우지 않는다. 여러 장치를 사용했다면 각 장치의 마지막 JSON을 모두 모은다.
...
- 여섯 레벨 각각 서로 다른 테스트 ID 3명의 완주가 있어야 한다.
- 각 레벨에 일반 2회와 쉬움 1회, 실세아와 감자89, 키보드와 게임패드가 포함되어야 한다.
- 전체 목표는 일반 6~9분, 쉬움 5~8분이다. 다섯 보스전의 phase 합계는 최초 기준 60~90초로 비교하되 판독성과 재미를 우선하며, 무보스 `level-06`은 공중 게이트까지의 코스 완주 시간과 20초 정체만 따로 본다.
...
18개 JSON과 관찰표가 모이기 전에는 실제 플레이테스트 항목을 완료로 표시하지 않는다. 2026-09-03 사용자가 `최종 플레이테스트 미실시 예외 승인`을 명시했으므로 기존 Phase 4 게이트는 승인 종료하되, G7에서도 미실시 항목은 수행 완료로 바꾸지 않는다. 향후 테스트를 재개하면 이 가이드와 새 JSON을 그대로 사용한다.
