...
> [!NOTE]
> **ARCHIVED DOCUMENT**  
> 이 문서는 Phase 3 에셋 생성 당시 사용된 AI 이미지 프롬프트 목록입니다.  
> **현재 모든 인게임 에셋은 렌더링 및 조립이 완료되어 보관(Archived)**되었습니다.  
> 실제 등록된 에셋 키와 사양은 [ASSET_LIST.md](./ASSET_LIST.md) 및 [assets/manifest.json](./assets/manifest.json)을 참조하시기 바랍니다.
...
| `rainbow_gate` | 캐릭터가 통과할 넓은 빈 공간, 빛나는 바깥 테두리와 별 반짝임이 있는 둥근 아치 | 310108 | mapping | 2026-08-24 손그림 논의 반영·앵커 갱신 완료(광륜·간헐적 부유·반짝임은 런타임 분리) |
...
|---|---|---:|---|---|
| `decor_flower` | 128×128, 둥근 꽃잎 5장과 짧은 잎 2장, 낮은 채도 분홍·황토·환경 녹색, 후광·가시·텍스트 없음, 투명 배경 | 510101 | mapping | S6 완료 |
| `decor_grass` | 192×128, 높이가 다른 둥근 잎 5~7개가 겹친 낮은 풀 덤불, 환경 녹색 3톤, 뾰족한 끝·후광 없음, 투명 배경 | 510102 | mapping | S6 완료 |
| `decor_rock` | 160×128, 높이 72px 이하의 비대칭 둥근 바위, 중성 회색 3톤과 좌상단 광원, 가시·균열·위험색 없음, 투명 배경 | 510103 | mapping | S6 완료 |
| `decor_sign` | 160×192, 오른쪽을 가리키는 둥근 화살표 판과 짧은 기둥, 목재색 3톤, 문자·숫자·위험색 없음, 투명 배경 | 510104 | mapping | S6 완료 |
...
- 편집 입력: `assets/_source/p12/p12_water_king_anchor_generated_v1.png`
- 채택 원본: `assets/_source/p12/p12_water_king_color_source_v1.png`
- 생성 방식: Codex 내장 이미지 생성 정밀 편집 모드.
- 요청: 승인된 세로 물방울 몸·짧은 팔다리·작은 기울어진 왕관·표정·포즈·접촉 시트 배치를 유지하고 몸 `#3DBFE3/#4691A2`, 물 `#9ADDF2`, 얼굴 `#F4FBFD`, 외곽선 `#42474E`, 왕관·별 `#F5DF4F/#D09A4E`만 적용한다.
...
2026-09-13 물에 잠긴 마을 배경 3종은 [동화풍 배경 원본 기록](assets/_source/submerged/storybook/README.md)의 생성 프롬프트를 사용합니다. 선택 썸네일의 선명한 수중 마을 화풍을 참조하며 배경 양자화·톤 치환은 적용하지 않습니다.
